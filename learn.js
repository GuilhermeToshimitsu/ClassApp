...
function eliminav(linha){//troca as virgulas que estão como elementos de string por /
...
function ArrumaLinhasRepetidas(csv, pos){//elimina linhas com o id descrito em pos, iguais, ele merge caso os elementos sejam iguais, elimina a linha repetida com *** e ''
	var linhas = csv.split('\n');
	var nlinhas = linhas.length;
...
function telephone(string){//retorna '' se o telefone nao for valido, para maior informações leia o README
...
function Arruma(csv){//organiza no formato de objeto desejado, não inclue address com valor nulo, e coloca see_all einvisible como default false
	var lines = csv.split("\n");
	var result = [];
...
var lin = datalinha.length;//numero de linhas na tabela(nro de elementos + especificacao)
for(j=0;j<lin;j++){
	datalinha[j] = eliminav(datalinha[j]);	//elimina virgulas indesejadas do input.
}

var temp = datalinha.join('\n');//refaz a string sem virgulas,
var final1 = ArrumaHeaderRepetido(temp);//procura colunas repetedias e merge com / 
var final2 = ArrumaLinhasRepetidas(final1,1);//procura linhas com o id (1) iguais, no caso seria a chave da posicao[1]
var final3 = Arruma(final2);//organiza no formato object desejado.
var final4 = JSON.stringify(final3, null,'\t');//transforma em string para passagem para o output.json
